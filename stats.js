...
export default requireAdmin(async (req, res) => {
  ...
  for (let i=5;i>=0;i--) {
    ...
    monthly.push({
      label,
      deposits: txs?.filter(t=>{const td=new Date(t.date);return t.type==='deposit'&&td.getFullYear()===y&&td.getMonth()===m}).reduce((a,t)=>a+t.amount,0)||0,
      withdrawals: txs?.filter(t=>{const td=new Date(t.date);return t.type==='withdrawal'&&td.getFullYear()===y&&td.getMonth()===m}).reduce((a,t)=>a+t.amount,0)||0,
    })
  }

  return res.status(200).json({
    totalUsers: users?.length||0, totalDeposits, totalWithdrawals, totalBalance,
    todayBets: todayBets?.length||0, monthly,
    topUsers: [...(users||[])].sort((a,b)=>b.balance-a.balance).slice(0,5),
