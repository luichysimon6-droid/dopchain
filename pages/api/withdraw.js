...
export default requireAuth(async (req, res) => {
  ...
  if (req.method === 'GET') {
    let q = supabaseAdmin.from('withdrawal_requests').select('*,users(name,email)').order('created_at', { ascending: false })
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabaseAdmin.from('withdrawal_requests').insert({ user_id: req.user.id, amount: parseFloat(amount), bank_name, account_number, account_name, status: 'pending' }).select().single()
    if (error) { await supabaseAdmin.from('users').update({ balance: u.balance }).eq('id', req.user.id); return res.status(500).json({ error: error.message }) }
